...
import type { Level, Region, Selection, GameStatus, RegionDef } from "../types/shikaku.types"
...
const PASTEL_COLORS = [
  "#ffd6d6", "#d0e8ff", "#c8f5d8", "#fff3c8", "#e4d0ff",
  "#ffd0ec", "#c8f5ee", "#ffe4cc", "#cce8ff", "#ecffc8",
  "#fce0cc", "#ccf5e4", "#f5ccf0", "#e0f5cc", "#ccd4f5",
]
...
export function useShikakuGame(initialLevel: Level) {
  ...
  const placeRegion = useCallback((sel: Selection): boolean => {
    const result = validateRectangle(sel, level.clues, regions, level.rows, level.cols)
