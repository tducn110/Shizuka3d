...
function deriveShades(hex: string) {
  ...
  return {
    top:   toHex(r + (255 - r) * 0.22, g + (255 - g) * 0.22, b + (255 - b) * 0.22),
    right: hex,
...
export default function IsometricBoard({
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      for (const state of animH.current.values()) {
        ...
        if (Math.abs(d) > 0.001) { state.curr += d; dirty = true }
      }
  ...
  const clampCell = useCallback((col: number, row: number) => ({
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent<SVGSVGElement>) => {
    ...
    setSelection({
      startRow: startCell.current.row, startCol: startCell.current.col,
      endRow: cell.row, endCol: cell.col,
    })
  }, [getCell])

  const handlePointerUp = useCallback((e: React.PointerEvent<SVGSVGElement>) => {
    if (!dragging.current || !startCell.current) { dragging.current = false; return }
    ...
    if (!valid) {
      setSelState("invalid")
      setTimeout(() => { setSelection(null); setSelState("dragging") }, 380)
    } else {
      setSelection(null)
    }
  ...
  const handlePointerCancel = useCallback(() => {
    dragging.current = false; startCell.current = null
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          <filter id="ibShadow">
            <feDropShadow dx="0" dy={S * 0.12} stdDeviation={S * 0.18} floodColor="#00000020" />
          </filter>
        </defs>
        ...
        <g filter="url(#ibShadow)">
          {sortedRegions.map(region => {
            const h = animH.current.get(region.id)?.curr ?? boxH(region.clueValue)
            const { dx, dy } = parallax(h)
            const { top, right, left } = deriveShades(region.color)
            const c0 = region.col, r0 = region.row
            const c1 = c0 + region.width, r1 = r0 + region.height

            return (
              <g key={region.id} transform={`translate(${dx.toFixed(2)},${dy.toFixed(2)})`}>
        ...
        {hintRegion && (() => {
          const h = 0.35
          const { dx, dy } = parallax(h)
          const c0 = hintRegion.col, r0 = hintRegion.row
          const c1 = c0 + hintRegion.width, r1 = r0 + hintRegion.height
          return (
            <g transform={`translate(${dx.toFixed(2)},${dy.toFixed(2)})`} style={{ animation: "hintGlow 0.65s ease-in-out infinite alternate" }}>
              <polygon
                points={`${pt(c0,r0,h)} ${pt(c1,r0,h)} ${pt(c1,r1,h)} ${pt(c0,r1,h)}`}
        ...
        {selRect && (() => {
          const h = 0.28
          const { dx, dy } = parallax(h)
          const c0 = selRect.c0, r0 = selRect.r0, c1 = selRect.c1 + 1, r1 = selRect.r1 + 1
          const isInvalid = selState === "invalid"
          const fillColor = isInvalid ? "rgba(230,60,60,0.25)" : "rgba(90,130,240,0.22)"
          const strokeColor = isInvalid ? "rgba(210,50,50,0.6)" : "rgba(80,120,220,0.55)"
          return (
            <g
